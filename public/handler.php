...
/* $Id: handler.php,v 1.49 2007/12/17 08:45:31 liedekef Exp $ */
 ...
 */
	...
	$GLOBALS['ESPCONFIG']['limit_double_postings'] = 0;
	...
	if(!empty($_REQUEST['submit'])) {
		...
		if(empty($msg)) {
			...
			if ($_POST['stype'] != 'pre')
				...
				goto_thankyou($sid,"/admin/manage.php");
			else
				goto_action_url($_GET['project']);
			return;
		}
	}
...
?>
<form method="post" id="phpesp_response" action="<?php echo($action); ?><?php if(isset($_GET['project'])) /*if(eregi('?',$action)) echo '&project='; else echo '?project='; echo $_GET['project']; */ echo '&project='.$_GET['project'];?>">
...
<input type="hidden" name="stype" value="<?php if(isset($stype)) echo $stype; else echo "pre";?>" />
<input type="hidden" name="vid" value="<?php $vid; ?>2" />
    </fieldset>
<?php
...
<?php
		...
		if($_REQUEST['sec'] == $num_sections) {
            ...
            //http://130.63.175.158/public/gluebox.php?project=98#finish
